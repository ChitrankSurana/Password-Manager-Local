        ...
        if required_version != "any" and hasattr(module, '__version__'):
            ...
            try:
                ...
                if version.parse(installed_version) >= version.parse(required_version):
                    ...
                    print_colored("    [ERROR] Version too old", Colors.RED)
                    return False
            except ImportError:
                # If packaging is not available, assume version is OK
                print_colored("    ? Cannot verify version (assuming OK)", Colors.YELLOW)
                return True
        else:
...
def check_system_requirements() -> bool:
    ...
    if system in supported_systems:
        print_colored("[OK] Operating system is supported", Colors.GREEN)
        system_ok = True
    else:
        print_colored("? Operating system may not be fully supported", Colors.YELLOW)
    ...
    try:
        ...
        if free_space >= 1.0:  # At least 1 GB free space
            ...
            print_colored(
                "[WARNING] Low disk space - may cause issues with database and backups",
                Colors.YELLOW)
