...
def initialize_enhanced_services(password_manager, auth_manager):
    ...
    try:
        ...
        database_manager = (
            password_manager.database_manager
            if hasattr(password_manager, "database_manager")
            else None
        )
        ...
        service_integrator = create_service_integrator(
            database_manager=database_manager, config=service_config
        )
        ...
        logging.info(
            f"Enhanced services initialized successfully - Status: {health_status['overall_status']}"
        )
        ...
        def on_login_success(session_id: str, username: str, master_password: str = None):
            ...
            try:
                ...
                def on_main_window_close():
                    cleanup_application(
                        session_id, auth_manager, main_window, service_integrator, root
                    )
...
def parse_arguments():
    ...
    interface_group.add_argument(
        "--gui", action="store_true", help="Launch enhanced GUI interface (default)"
    )

    interface_group.add_argument("--web", action="store_true", help="Launch web interface")

    interface_group.add_argument(
        "--check-deps", action="store_true", help="Run comprehensive dependency checker"
    )
