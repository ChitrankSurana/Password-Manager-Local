...
def show_passwords(user_id=None, show_encrypted=False):
    ...
    print("\n" + "=" * 120)
    print("🔐 PASSWORD ENTRIES" + (f" (User ID: {user_id})" if user_id else " (All Users)"))
    print("=" * 120)
    ...
    if not passwords:
        ...
        for pwd in passwords:
            encrypted_preview = (
                pwd["password_encrypted"][:20] + "..." if show_encrypted else "[ENCRYPTED]"
            )
    ...
    try:
        ...
        if not metadata:
            ...
            for item in metadata:
                metadata_data.append([item["key"], item["value"]])

            print(tabulate(metadata_data, headers=["Key", "Value"], tablefmt="grid"))

    except sqlite3.Error as e:
        print("  ℹ️  No metadata table found (this is normal for older databases)")
...
def verify_data_integrity():
    ...
    cursor.execute(
        "SELECT COUNT(*) FROM passwords WHERE password_encrypted IS NULL OR password_encrypted = ''"
    )
