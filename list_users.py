...
def list_users(db_path: str = "data/password_manager.db") -> List[Dict[str, Any]]:
    ...
    if not db_file.exists():
        ...
        print(f"It will be created when you run the application for the first time.")
        return []
    ...
    try:
        ...
        if not cursor.fetchone():
            print(f"[ERROR] Database exists but 'users' table not found.")
            print(f"This might be a corrupted or incompatible database file.")
            conn.close()
            return []
    ...
    for i, user in enumerate(users, 1):
        ...
        if user['locked_until']:
            ...
            if datetime.now() < locked_until:
                print(f"  - Status:         [LOCKED] until {format_timestamp(user['locked_until'])}")
            else:
                print(f"  - Status:         [Lock expired - will be cleared on next login]")
