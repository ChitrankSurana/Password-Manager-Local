...
class EnhancedMainWindow(MainWindow):
    ...
    def __init__(
        ...
        logger.info(
            f"Enhanced main window initialized - Services available: {self.services_available}"
        )
        ...
        try:
            ...
            if hasattr(self, "status_label"):
                ...
                self.service_status_label = create_themed_label(
                    service_frame, text="🔧 Services: Checking...", style="label_secondary"
                )
    ...
    def show_password_view_auth_dialog(self):
        ...
        if not self.services_available:
            messagebox.showwarning(
                "Feature Not Available", "Enhanced password viewing requires service integration."
            )
            return None
        ...
        try:
            ...
            messagebox.showerror(
                "Authentication Error", f"Could not show authentication dialog: {e}"
            )
            return None
    ...
    def get_security_dashboard_data(self, hours: int = 24):
        ...
        try:
            return self.service_integrator.get_security_dashboard(
                user_id=self.enhanced_user_id, hours=hours
            )
    ...
    def search_security_events(self, filters: Dict[str, Any] = None, limit: int = 100):
        ...
        try:
            return self.service_integrator.search_security_events(filters or {}, limit)
    ...
    def _on_entries_loaded(self, entries):
        ...
        if self.services_available and self.service_integrator._security_audit_logger:
            try:
                from ..core.security_audit_logger import EventResult, SecurityEventType
        ...
        if self.services_available and msg_type in ["error", "warning"]:
            try:
                ...
                self.service_integrator._security_audit_logger.log_event(
                    ...
                    self.enhanced_session_id,
                    result,
                    event_details={"ui_message": message, "message_type": msg_type},
                )
