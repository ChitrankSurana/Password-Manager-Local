...
from ..core.settings_service import SettingCategory
...
class SettingsWindow(ctk.CTkToplevel):
    ...
    def _create_ui_preferences_tab(self):
        ...
        self._create_font_size_setting(
            scroll_frame, category, "font_size",
            category_settings, category_definitions
        )
    ...
    def _create_timeout_setting(self, parent, category: str, settings: Dict, definitions: Dict):
        ...
        slider = ctk.CTkSlider(
            ...
            number_of_steps=setting_def.max_value -
            setting_def.min_value,
            command=lambda value: self._update_timeout_display(
                value,
                value_label,
                category,
                "view_timeout_minutes"))
        ...
        for minutes in [1, 5, 15, 30]:
            btn = create_themed_button(
                ...
                width=60,
                command=lambda m=minutes: self._set_quick_timeout(
                    slider,
                    value_label,
                    m,
                    category,
    ...
    def _create_security_checkbox_setting(
            self,
            parent,
            category: str,
            key: str,
        ...
        checkbox = ctk.CTkCheckBox(
            ...
            text=setting_def.display_name,
            variable=checkbox_var,
            command=lambda: self._on_security_setting_changed(
                category,
                key,
                checkbox_var.get(),
                checkbox_var))
    ...
    def _create_font_size_setting(
            ...
            settings: Dict,
            definitions: Dict):
        """Create font size dropdown setting with preview"""
        setting_def = definitions.get(key)
        if not setting_def:
            return
        ...
        # Dropdown with friendly labels
        font_size_labels = {
            "small": "Small (90%)",
            "medium": "Medium (100%)",
            "large": "Large (110%)",
            "extra_large": "Extra Large (120%)"
        }

        # Create display values for dropdown
        display_values = [font_size_labels.get(val, val) for val in setting_def.allowed_values]
        current_display = font_size_labels.get(current_value, current_value)

        dropdown_var = tk.StringVar(value=current_display)
        dropdown = ctk.CTkComboBox(
            label_frame,
            variable=dropdown_var,
            values=display_values,
            command=lambda value: self._on_font_size_changed(
                category,
                key,
                value,
                font_size_labels,
        ...
        # Preview section
        preview_frame = ctk.CTkFrame(setting_frame, fg_color="transparent")
        preview_frame.pack(fill="x", padx=15, pady=(0, 10))

        preview_title = create_themed_label(
            preview_frame,
            text="Preview:",
            style="label_secondary"
        )
        ...
        # Font size preview with dynamic size
        font_size_map = {"small": 13, "medium": 14, "large": 15, "extra_large": 17}
        preview_size = font_size_map.get(current_value, 14)

        preview_label = create_themed_label(
            preview_frame,
            text="The quick brown fox jumps over the lazy dog",
            style="label"
        )
        preview_label.configure(font=("Segoe UI", preview_size))
        preview_label.pack(anchor="w", pady=(0, 5))

        # Restart notice
        restart_notice = create_themed_label(
            setting_frame,
            text="ℹ️ Note: Font size changes will take effect when you restart the application.",
            style="label_secondary"
        )
        restart_notice.configure(wraplength=500, text_color="#0066cc")
        restart_notice.pack(fill="x", padx=15, pady=(0, 10))

        # Store control reference
        self.setting_controls[f"{category}.{key}"] = dropdown_var
        ToolTip(dropdown, setting_def.description or "")

    def _create_advanced_setting_section(
            self,
            parent,
            category: str,
            key: str,
    ...
    def _on_security_setting_changed(self, category: str, key: str, value: Any, checkbox_var):
        ...
        if value:
            ...
            result = messagebox.askyesno(
                "Security Setting",
                "Enabling this security setting will affect password deletion behavior.\n\n"
    ...
    def _on_font_size_changed(
            ...
            key: str,
            display_value: str,
            font_size_labels: Dict,
            preview_label):
        """Handle font size setting change with live preview"""
        # Convert display label back to actual value
        value_map = {v: k for k, v in font_size_labels.items()}
        actual_value = value_map.get(display_value, "medium")

        # Update preview label
        font_size_map = {"small": 13, "medium": 14, "large": 15, "extra_large": 17}
        preview_size = font_size_map.get(actual_value, 14)
        preview_label.configure(font=("Segoe UI", preview_size))

        # Mark setting as changed
        self._on_setting_changed(category, key, actual_value)
    ...
    def _apply_changes(self):
        ...
        try:
            ...
            if errors:
                ...
                messagebox.showinfo(
                    "Success", f"Applied {changes_applied} setting changes successfully.")
