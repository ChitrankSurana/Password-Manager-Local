...
class DatabaseMigrationManager:
    ...
    def _register_migrations(self):
        ...
        # Migration from version 3 to version 4: Add two-factor authentication support
        self._migrations[4] = self._migrate_to_version_4

        # Migration from version 4 to version 5: Add performance indexes
        self._migrations[5] = self._migrate_to_version_5
    ...
    def needs_migration(self) -> bool:
        ...
        if needs_update:
            logger.info(
                f"Migration needed: current version {current_version}, latest version {latest_version}"
            )
        else:
    ...
    def _migrate_to_version_2(self, conn: sqlite3.Connection):
        ...
        if existing_users:
            ...
            all_default_settings = (
                default_viewing_settings + default_deletion_settings + default_security_settings
            )
    ...
    def _migrate_to_version_4(self, conn: sqlite3.Connection):
        """
        Migration from version 3 to version 4: Add two-factor authentication support

        This migration adds:
        1. totp_secret column to users table for TOTP secret keys
        2. totp_enabled column to users table to enable/disable 2FA
        3. backup_codes column to users table for recovery codes
        ...
        logger.info("Starting migration to version 4: Adding two-factor authentication support")
        ...
        try:
            ...
            if "duplicate column" in str(e).lower():
                logger.warning("totp_secret column already exists, skipping column addition")
            else:
                raise

        # 2. Add totp_enabled column to users table
        try:
            cursor.execute(
                """
            ...
            if "duplicate column" in str(e).lower():
                logger.warning("totp_enabled column already exists, skipping column addition")
            else:
                raise

        # 3. Add backup_codes column to users table (stored as JSON array)
        try:
            cursor.execute(
                """
            ...
            if "duplicate column" in str(e).lower():
                logger.warning("backup_codes column already exists, skipping column addition")
            else:
                raise

        # 4. Create index for 2FA enabled users for faster queries
        cursor.execute(
            """
                    ...
                    json.dumps(
                        {
                            ...
                            "columns_added": ["totp_secret", "totp_enabled", "backup_codes"],
                            "indexes_added": ["idx_users_totp_enabled"],
                            "description": "Added two-factor authentication support with TOTP and backup codes",
                        }
                    ),
                ),
            )
    ...
    def _migrate_to_version_5(self, conn: sqlite3.Connection):
        """
        Migration from version 4 to version 5: Add performance indexes

        This migration adds indexes for frequently queried and sorted columns
        to improve search and filter performance, especially for large databases.

        Indexes added:
        1. idx_passwords_website - For faster website lookups
        2. idx_passwords_created_at - For date range queries and sorting
        3. idx_passwords_modified_at - For "recently modified" sorting
        4. idx_passwords_is_favorite - For filtering favorites
        5. idx_passwords_username_search - For username searches
        ...
        logger.info("Starting migration to version 5: Adding performance indexes")

        # 1. Add index on website column for faster website searches
        cursor.execute(
            """
            CREATE INDEX IF NOT EXISTS idx_passwords_website
            ON passwords(website)
        """
        )
        logger.info("Created index on website column")

        # 2. Add index on created_at for date range queries and sorting
        cursor.execute(
            """
            CREATE INDEX IF NOT EXISTS idx_passwords_created_at
            ON passwords(created_at DESC)
        """
        )
        logger.info("Created index on created_at column")

        # 3. Add index on modified_at for "recently modified" sorting
        cursor.execute(
            """
            CREATE INDEX IF NOT EXISTS idx_passwords_modified_at
            ON passwords(modified_at DESC)
        """
        )
        logger.info("Created index on modified_at column")

        # 4. Add index on is_favorite for filtering favorites
        cursor.execute(
            """
            CREATE INDEX IF NOT EXISTS idx_passwords_is_favorite
            ON passwords(is_favorite)
        """
        )
        logger.info("Created index on is_favorite column")

        # 5. Add composite index for user_id + modified_at for user-specific recent queries
        cursor.execute(
            """
            CREATE INDEX IF NOT EXISTS idx_passwords_user_modified
            ON passwords(user_id, modified_at DESC)
        """
        )
        logger.info("Created composite index on user_id and modified_at")

        # 6. Add index on username for faster username searches
        cursor.execute(
            """
            CREATE INDEX IF NOT EXISTS idx_passwords_username_search
            ON passwords(username)
        """
        )
        logger.info("Created index on username column")

        # 7. Run ANALYZE to update query planner statistics
                    ...
                    json.dumps(
                        {
                            ...
                            "indexes_added": [
                                "idx_passwords_website",
                                "idx_passwords_created_at",
                                "idx_passwords_modified_at",
                                "idx_passwords_is_favorite",
                                "idx_passwords_user_modified",
                                "idx_passwords_username_search",
                            ],
                            "description": "Added performance indexes for faster search, sort, and filter operations",
                        }
                    ),
                ),
            )
