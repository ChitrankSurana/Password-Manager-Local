...
from .error_handlers import audit_action, handle_errors, handle_security_errors, monitor_performance

# Import new error handling system
from .exceptions import AccountLockedError
from .exceptions import AuthenticationError as NewAuthenticationError
...
from .exceptions import SessionExpiredError as NewSessionExpiredError
from .logging_config import get_logger, log_audit_event, log_exception, log_security_event
...
AuthenticationError = NewAuthenticationError
SessionExpiredError = NewSessionExpiredError
InvalidSessionError = NewSessionExpiredError  # Map to SessionExpiredError
...
class UserSession:
    ...
    pending_2fa: bool = field(default=False)  # True if waiting for 2FA validation
...
class AuthenticationManager:
    ...
    def __init__(
        self, db_path: str = "data/password_manager.db", session_timeout_hours: int = None
    ):
        """
        ...
        self._cleanup_thread = threading.Thread(
            target=self._session_cleanup_worker, daemon=True, name="SessionCleanup"
        )
        self._cleanup_thread.start()

        logger.info(
            f"Authentication manager initialized with {self.session_timeout_hours}h session timeout"
        )

    @handle_security_errors("User account creation failed")
    @audit_action("CREATE_USER_ACCOUNT", lambda args, kwargs: None)  # No user_id yet
    def create_user_account(self, username: str, master_password: str) -> int:
        ...
        try:
            ...
            log_security_event(
                "USER_ACCOUNT_CREATED", f"New user account: {username}", user_id=user_id
            )
            ...
            log_security_event(
                ...
                f"User creation failed for '{username}': {e}",
                severity="WARNING",
            )
            logger.warning(f"User creation failed for '{username}': {e}")
            raise AuthenticationError(str(e))
        except Exception as e:
            log_exception(logger, e, f"Unexpected error during user creation for '{username}'")
            raise AuthenticationError(f"Account creation failed: {e}")

    @handle_security_errors("User authentication failed")
    @monitor_performance(threshold_ms=1000)  # Alert if auth takes > 1s
    def authenticate_user(
        self,
        username: str,
        master_password: str,
        login_ip: str = "127.0.0.1",
        user_agent: str = "Desktop Application",
    ) -> str:
        """
        ...
        try:
            ...
            if user_session_count >= self.max_sessions_per_user:
                ...
                raise AuthenticationError(
                    f"Maximum {self.max_sessions_per_user} concurrent sessions allowed"
                )

            # Check if 2FA is enabled for this user
            twofa_status = self.db_manager.get_2fa_status(user_info["user_id"])
            ...
            session = UserSession(
                ...
                pending_2fa=twofa_status["enabled"],  # Mark as pending if 2FA enabled
            )
            ...
            if twofa_status["enabled"]:
                log_security_event(
                    "LOGIN_PENDING_2FA",
                    f"User password verified, awaiting 2FA: {username}",
                    user_id=user_info["user_id"],
                    ip_address=login_ip,
                )
                logger.info(
                    f"User password verified, pending 2FA: {username} (Session: {session_id[:8]}...)"
                )
            else:
                log_security_event(
                    "LOGIN_SUCCESS",
                ...
                log_audit_event(
                    "LOGIN",
                    user_info["user_id"],
                    details={"ip": login_ip, "user_agent": user_agent},
                )
                logger.info(
                    f"User authenticated successfully: {username} (Session: {session_id[:8]}...)"
                )
    ...
    @handle_security_errors("2FA validation failed")
    def validate_2fa_code(self, session_id: str, code: str, is_backup_code: bool = False) -> bool:
        """
        Validate a 2FA code (TOTP or backup code) and complete authentication

        Args:
            session_id: Session ID returned from authenticate_user
            code: 6-digit TOTP code or backup code
            is_backup_code: True if this is a backup code, False for TOTP

        Returns:
            bool: True if code is valid and authentication completed

        Raises:
            AuthenticationError: If validation fails
            InvalidSessionError: If session not found or not pending 2FA
        """
        # Get session
        with self._lock:
        ...
        if not session.pending_2fa:
            raise AuthenticationError("This session is not pending 2FA validation")

        try:
            # Get user's TOTP secret and backup codes
            totp_secret = self.db_manager.get_totp_secret(session.user_id)
            backup_codes_json = self.db_manager.get_backup_codes(session.user_id)

            if is_backup_code:
                # Validate backup code
                backup_codes = self.totp_service.load_backup_codes_from_storage(backup_codes_json)
                is_valid, remaining_codes = self.totp_service.verify_backup_code(
                    code, backup_codes, user_id=session.user_id, username=session.username
                )

                if is_valid:
                    # Update backup codes in database
                    updated_json = (
                        self.totp_service.prepare_backup_codes_for_storage(remaining_codes)
                        if remaining_codes
                        else None
                    )
                    self.db_manager.update_backup_codes(session.user_id, updated_json)
                    ...
                    log_security_event(
                        "2FA_SUCCESS_BACKUP",
                        f"2FA completed with backup code for user: {session.username}",
                    ...
                    logger.info(f"2FA completed with backup code: {session.username}")
                    return True
                else:
                    log_security_event(
                        "2FA_FAILED",
                        f"Invalid backup code for user: {session.username}",
                        severity="WARNING",
                        user_id=session.user_id,
                    )
                    return False
                ...
                is_valid = self.totp_service.verify_totp_code(
                    totp_secret, code, user_id=session.user_id, username=session.username
                )
                ...
                if is_valid:
                    ...
                    log_security_event(
                        "2FA_SUCCESS",
                        f"2FA completed successfully for user: {session.username}",
                        user_id=session.user_id,
                        ip_address=session.login_ip,
                    )
                    log_audit_event(
                    ...
                    logger.info(f"2FA completed successfully: {session.username}")
                    return True
                else:
                    log_security_event(
                        "2FA_FAILED",
                        f"Invalid TOTP code for user: {session.username}",
                        severity="WARNING",
                        user_id=session.user_id,
                    )
                    return False
            ...
            log_exception(logger, e, f"2FA validation error for user {session.username}")
            raise AuthenticationError(f"2FA validation failed: {e}")

    def is_session_pending_2fa(self, session_id: str) -> bool:
        """
        Check if a session is pending 2FA validation

        Args:
            session_id: Session ID to check

        Returns:
            bool: True if session exists and is pending 2FA
        """
        with self._lock:
            session = self.active_sessions.get(session_id)
        return session.pending_2fa if session else False
    ...
    def logout_user(self, session_id: str) -> bool:
        ...
        try:
            ...
            if session:
                ...
                log_security_event("LOGOUT", f"User logged out: {session.username}")
            ...
            log_exception(logger, e, "Error during logout")
            return False

    @handle_security_errors("Master password change failed")
    @audit_action("CHANGE_MASTER_PASSWORD", lambda args, kwargs: None)  # Will be logged separately
    @monitor_performance(threshold_ms=5000)  # Multiple crypto operations
    def change_master_password(
        self, session_id: str, current_password: str, new_password: str
    ) -> bool:
        """
        ...
        try:
            ...
            log_security_event(
                ...
                f"Master password changed for user: {session.username}",
                user_id=session.user_id,
                severity="INFO",
            )
            log_audit_event(
                ...
                session.user_id,
                details={"passwords_re_encrypted": len(updated_entries)},
            )
            ...
            log_exception(logger, e, "Master password change failed")
            raise AuthenticationError(f"Password change failed: {e}")

    def get_user_password_entries(
        self, session_id: str, website: str = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            logger.debug(
                f"Retrieved {len(safe_entries)} password entries for user {session.username}"
            )
            return safe_entries
    ...
    def add_password_entry(
        self, session_id: str, website: str, username: str, password: str, remarks: str = ""
    ) -> int:
        """
...
def create_auth_manager(
    db_path: str = "data/password_manager.db", session_timeout_hours: int = 8
