...
class ViewPermissionGrant:
    ...
    def __init__(self, session_id: str, user_id: int, timeout_minutes: int,
                 auth_method: AuthenticationMethod, granted_by_ip: str = "127.0.0.1"):
        """
...
class PasswordViewAuthService:
    ...
    def __init__(self, default_timeout_minutes: int = 1):
        ...
        logger.info(f"PasswordViewAuthService initialized with {default_timeout_minutes}min default timeout")
    
    def grant_view_permission(self, session_id: str, user_id: int, 
                            master_password_hash: str, provided_password_hash: str,
                            timeout_minutes: Optional[int] = None,
                            client_ip: str = "127.0.0.1") -> ViewPermissionGrant:
        """
        ...
        with self._lock:
            ...
            if session_id in self._permissions:
                old_permission = self._permissions[session_id]
            ...
            logger.info(f"View permission granted to session {session_id[:8]}... for {timeout} minutes")
            return permission
    ...
    def extend_permission(self, session_id: str, additional_minutes: int) -> bool:
        ...
        with self._lock:
            ...
            if success:
                logger.info(f"Extended permission for session {session_id[:8]}... by {additional_minutes} minutes")
    ...
    def get_permission_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                'service_uptime_minutes': int((datetime.now() - self._last_cleanup).total_seconds() / 60),
        ...
        try:
            ...
            if current_lock_state != self._system_lock_detected:
                ...
                if current_lock_state:
                    ...
                    logger.warning(f"System lock detected - revoked {revoked_count} view permissions")
                    return True
                else:
    ...
    def _revoke_permission(self, session_id: str, reason: str) -> bool:
        ...
        if permission:
            ...
            logger.debug(f"View permission revoked for session {session_id[:8]}... (reason: {reason})")
            return True
    ...
    def _verify_password_hash(self, stored_hash: str, provided_hash: str) -> bool:
        ...
        return len(stored_hash) == len(provided_hash) and \
               sum(a != b for a, b in zip(stored_hash, provided_hash)) == 0
    ...
    def _log_auth_attempt(self, session_id: str, user_id: int, result: str, 
                         client_ip: str, details: Optional[Dict] = None):
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"Testing PasswordViewAuthService...")
