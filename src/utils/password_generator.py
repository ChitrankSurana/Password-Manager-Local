...
class PasswordGenerator:
    ...
    def generate_password(self, options: GenerationOptions = None,
        ...
        try:
            ...
            logger.debug(
                f"Generated password using {
                    method.value} method, strength: {
    ...
    def _generate_with_requirements(self, charset: str, options: GenerationOptions) -> str:
        ...
        for attempt in range(max_attempts):
            ...
            if not options.require_each_type or self._meets_character_requirements(
                    password, options):
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            f"  Length: {len(random_password.password)}, Entropy: {random_password.entropy:.1f} bits")

        # Test memorable password generation
        memorable_options = GenerationOptions(word_count=3, add_numbers=True)
        memorable_password = generator.generate_password(
            memorable_options, GenerationMethod.MEMORABLE)
