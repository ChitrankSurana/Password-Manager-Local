...
def run_test_suite(test_suite, verbose=False):
    ...
    runner = ColoredTextTestRunner(
        stream=stream if not verbose else sys.stdout,
...
def run_all_tests(verbose=False, with_coverage=False):
    ...
    return all(result.failed_tests == 0 and result.error_tests == 0 
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Run tests for Personal Password Manager'
    )
